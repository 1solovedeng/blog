...
*/
/* 替换：buildScythePolygon — 两个矩形：shaft（棍） + blade（矩形刀片） */
function buildScythePolygon(p, resolution) {
  // resolution 参数不再用于生成半圆，但保留以兼容调用签名
  ...
  const nx = -diry, ny = dirx; // 法线（垂直于棍子方向）

  // 配置比例（可由 player 覆盖）
  const shaftRatio = (typeof p.scytheShaftRatio === 'number') ? p.scytheShaftRatio : 0.55; // 棍子占总长的比例
  const bladeRatio = (typeof p.scytheBladeRatio === 'number') ? p.scytheBladeRatio : 0.45; // 刀片“展宽”参考长度比例（与 total len 比例）
  const bladeDepthRatio = (typeof p.scytheBladeDepthRatio === 'number') ? p.scytheBladeDepthRatio : 0.12; // 刀片沿棍子方向的深度（相对于 weaponLength）

  const shaftLen = len * shaftRatio;
  const bladeLen = len * bladeRatio; // 刀片沿法线方向的总长（左右展开距离）
  const bladeDepth = len * bladeDepthRatio; // 刀片沿棍子方向的厚度（从背面到前缘）
  const halfShaftW = (p.weaponThickness || 14) / 2;

  // shaft 的端点（半圆原先的位置）
  const shaftEnd = { x: base.x + dirx * shaftLen, y: base.y + diry * shaftLen };

  // 棍子矩形四角（以 base->shaftEnd 的线为中心线）
  const leftBase  = { x: base.x + nx * halfShaftW, y: base.y + ny * halfShaftW };
  const rightBase = { x: base.x - nx * halfShaftW, y: base.y - ny * halfShaftW };
  const leftEnd   = { x: shaftEnd.x + nx * halfShaftW, y: shaftEnd.y + ny * halfShaftW };
  const rightEnd  = { x: shaftEnd.x - nx * halfShaftW, y: shaftEnd.y - ny * halfShaftW };

  // 刀片矩形（背面紧贴 shaftEnd，刀片朝外展开）
  // 刀片的中心沿棍子方向向外移动 bladeDepth/2，刀片沿法线方向展开 bladeLen/2
  const bladeHalfLen = bladeLen / 2;
  const bladeHalfDepth = bladeDepth / 2;

  // 刀片矩形 center（从 shaftEnd 向外移动一半深度）
  const bladeCenter = { x: shaftEnd.x + dirx * bladeHalfDepth, y: shaftEnd.y + diry * bladeHalfDepth };

  // 刀片四个角（顺序： 左背、左前、右前、右背 — 方便后面拼接）
  // 注意：左/右相对于法线方向 nx,ny（左 = +nx）
  const bladeLeftBack  = { x: bladeCenter.x + nx * bladeHalfLen - dirx * bladeHalfDepth, y: bladeCenter.y + ny * bladeHalfLen - diry * bladeHalfDepth };
  const bladeLeftFront = { x: bladeCenter.x + nx * bladeHalfLen + dirx * bladeHalfDepth, y: bladeCenter.y + ny * bladeHalfLen + diry * bladeHalfDepth };
  const bladeRightFront= { x: bladeCenter.x - nx * bladeHalfLen + dirx * bladeHalfDepth, y: bladeCenter.y - ny * bladeHalfLen + diry * bladeHalfDepth };
  const bladeRightBack = { x: bladeCenter.x - nx * bladeHalfLen - dirx * bladeHalfDepth, y: bladeCenter.y - ny * bladeHalfLen - diry * bladeHalfDepth };

  // 组合多边形点序： leftBase -> leftEnd -> bladeLeftBack -> bladeLeftFront ->
  //                     bladeRightFront -> bladeRightBack -> rightEnd -> rightBase
  // 这样得到的多边形可能是非凸的，但用于 pointInPolygon / 边相交检测是足够的
  const poly = [];
  poly.push(leftBase);
  poly.push(leftEnd);
  poly.push(bladeLeftBack);
  poly.push(bladeLeftFront);
  poly.push(bladeRightFront);
  poly.push(bladeRightBack);
  poly.push(rightEnd);
  poly.push(rightBase);
