/* game.js - Weapon-balls full script
   - No front-matter (avoid Hexo YAML parse).
   - Scythe = stick + semicircle hit polygon.
   - Staff fireball default radius small (can override per player).
   - ballbounce sound played when projectiles hit bounds/obstacles.
*/
...
function distancePointToSegmentSquared(p, v, w) {
  const lx = w.x - v.x, ly = w.y - v.y;
  const l2 = lx * lx + ly * ly;
  if (l2 === 0) {
    const dx = p.x - v.x, dy = p.y - v.y;
    return dx * dx + dy * dy;
  }
  let t = ((p.x - v.x) * lx + (p.y - v.y) * ly) / l2;
  t = Math.max(0, Math.min(1, t));
  const projx = v.x + t * lx;
  const projy = v.y + t * ly;
...
function registerSound(key, src, opts = {}) {
  try {
    ...
    console.warn('registerSound failed', key, src, e);
  }
}
...
/* ---------------- Defaults (include ballbounce, staff, fireball small) ---------------- */
(function setupDefaults() {
  ...
  registerWeapon({ key: 'staff', name: 'Staff', texture: 'staff.png', thickness: 8, baseRange: 48, projectile: 'fireball' });

  registerProjectile({ key: 'arrow', texture: 'arrow.png', radius: 4, speed: 0.5 });
  // make default fireball smaller — change radius here to tune globally
  registerProjectile({ key: 'fireball', texture: 'fireball.png', radius: 6, speed: 0.35 });

  const sndBase = '/game/weaponballs/assets/sounds/';
  registerSound('hit', sndBase + 'hit.mp3');
  registerSound('swordsclash', sndBase + 'swordsclash.mp3');
  registerSound('arrow', sndBase + 'arrow.mp3');
  registerSound('ballbounce', sndBase + 'ballbounce.mp3');
})();
...
function loadAllTexturesAndSounds(baseTexturePath = '/game/weaponballs/assets/textures/') {
  ...
  weaponKeys.forEach(k => {
    const filename = window.WEAPON_REGISTRY[k].texture;
    if (filename) {
      tasks.push(new Promise(resolve => {
        const img = new Image();
        img.crossOrigin = 'anonymous';
        img.onload = () => { window.WEAPON_TEXTURES[k] = img; resolve(); };
        img.onerror = () => { window.WEAPON_TEXTURES[k] = null; console.warn('texture load failed', k, filename); resolve(); };
        img.src = baseTexturePath + filename + '?_=' + Date.now();
      }));
    } else {
  ...
  projKeys.forEach(k => {
    const filename = window.PROJECTILE_REGISTRY[k].texture;
    if (filename) {
      tasks.push(new Promise(resolve => {
        const img = new Image();
        img.crossOrigin = 'anonymous';
        img.onload = () => { window.PROJECTILE_TEXTURES[k] = img; resolve(); };
        img.onerror = () => { window.PROJECTILE_TEXTURES[k] = null; console.warn('proj texture load failed', k, filename); resolve(); };
        img.src = baseTexturePath + filename + '?_=' + Date.now();
      }));
    } else {
...
/* ---------------- scythe polygon (stick + semicircle) ---------------- */
function buildScythePolygon(player, resolutionOverride) {
  ...
  const defaultScy = (window.WEAPON_REGISTRY && window.WEAPON_REGISTRY['scythe']) ? window.WEAPON_REGISTRY['scythe'].scythe : { shaftRatio: 0.55, bladeRadiusRatio: 0.45, thicknessMultiplier: 1.0, resolution: 20 };
  const shaftRatio = (typeof player.scytheShaftRatio === 'number') ? player.scytheShaftRatio : (defaultScy.shaftRatio || 0.55);
  const bladeRadiusRatio = (typeof player.scytheBladeRadiusRatio === 'number') ? player.scytheBladeRadiusRatio : (defaultScy.bladeRadiusRatio || 0.45);
  const thicknessMul = (typeof player.scytheThicknessMultiplier === 'number') ? player.scytheThicknessMultiplier : (defaultScy.thicknessMultiplier || 1.0);
  const res = (typeof player.scytheResolution === 'number') ? Math.max(6, Math.floor(player.scytheResolution)) : Math.max(6, Math.floor(resolutionOverride || defaultScy.resolution || 20));
...
class Game {
  ...
  init() {
    ...
    if (this.settings && Array.isArray(this.settings.spawnConfigs)) spawnConfigs = this.settings.spawnConfigs;
      ...
      { x: 0.75, y: 0.5, weaponType: 'staff', color: '#d46b15ff', health: 250 }
    ];
    ...
    spawnConfigs.forEach((cfg, index) => {
      ...
      if (p.weaponType === 'staff') {
        if (typeof p.fireballDamage !== 'number') p.fireballDamage = 2;
        // allow per-player override of fireball radius
        if (typeof p.fireballRadius !== 'number') {
          const defProj = window.PROJECTILE_REGISTRY['fireball'];
          p.fireballRadius = (defProj && defProj.radius) ? defProj.radius : 6;
        }
      }
  ...
  loop(timestamp) {
    ...
    if (survivors.length <= 1) { this.running = false; this.showGameOver(survivors[0]); } else requestAnimationFrame(this.loop.bind(this));
  }
  ...
  handleWeaponInteractions(time) {
    ...
    // Melee weapon hitting bodies (scythe uses polygon)
    for (let i = 0; i < this.players.length; i++) {
      const attacker = this.players[i];
      ...
      for (let j = 0; j < this.players.length; j++) {
        ...
        if (attacker.weaponType === 'scythe') {
          ...
          if (pointInPolygon({ x: target.x, y: target.y }, poly)) hit = true;
            ...
            const dsq = distancePointToSegmentSquared({ x: target.x, y: target.y }, lineStartA, tipA);
    ...
    // Weapon-to-weapon collisions (support scythe polygon)
    for (let i = 0; i < this.players.length; i++) {
  ...
  updateArrows(delta, time) {
    ...
    for (const arrow of this.arrows) {
      ...
      // boundary bounce sound and remove
  ...
  updateFireballs(delta, time) {
    for (const p of this.players) {
      ...
      if (p.fireballRadius === undefined) p.fireballRadius = (window.PROJECTILE_REGISTRY['fireball'] && window.PROJECTILE_REGISTRY['fireball'].radius) || 6;
      if (time - p.lastFireballTime >= p.fireballCooldown) {
        ...
        const projDef = window.PROJECTILE_REGISTRY[projKey] || { speed: 0.35, radius: p.fireballRadius || 6 };
        const speed = projDef.speed || 0.35;
        const vx = Math.cos(angle) * speed;
        const vy = Math.sin(angle) * speed;
        const start = p.getWeaponTip();
        const fb = { x: start.x, y: start.y, vx, vy, owner: p, damage: p.fireballDamage, radius: p.fireballRadius || projDef.radius || 6, projKey };
...
function installPlayerDrawOverride() {
  if (typeof Player === 'undefined') {
    console.warn('Player class not found - draw override skipped.');
    return;
  }
...
/* Usage notes:
 - To change fireball size globally: edit registerProjectile({ key:'fireball', radius: X })
 - To change only one staff player's radius: set spawn config property "fireballRadius" for that player
 - Ensure assets exist at the configured paths (textures and sounds).
 - If Hexo still complains about YAMLException, place this file under a directory not processed by Hexo or add that directory to _config.yml skip_render.
*/
